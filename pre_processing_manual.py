...
with open(DIR / "preproc_config.json") as file:
    cfg = json.load(file)
with open(DIR / "mapping.json") as file:
...
subj= "sub_28" #set to id/subject whose data you want to preprocess
...
#raw.set_eeg_reference('FCz')
raw.add_reference_channels('FCz') # Add reference electrode.
montage_path = DIR / cfg["montage"]["name"] # Get montage
...
epochs_clean.average().plot(exclude=["P4", "PO9"]) #manuell

bads=["P4", "PO9"] #Add channel names to exclude here
if len(bads) != 0:
   for bad in bads:
...
ax[0].set_title(f"Global Average, SNR={snr_pre:.2f}")
...
### STEP 6: Apply ICA ###
reference_path = DIR / cfg["ica"]["reference"]
...
ica.exclude = ["ICA000", "ICA001", ] #List of integer components to exclude
...
epochs_clean.save(
    epochs_folder / pathlib.Path("28-epo.fif"), overwrite=True)
